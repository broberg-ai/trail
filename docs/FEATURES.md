...
| F22 | [Stable `{#claim-xx}` Anchors](#f22-claim-anchors) | Done | 1 | [plan](features/F22-stable-claim-anchors.md) |
| F22.1 | [Sticky claim-anchor toggle in wiki-reader header](features/F22.1-sticky-claim-anchor-toggle.md) | Dropped | 1 | [plan](features/F22.1-sticky-claim-anchor-toggle.md) |
...
| F189 | [Upmetrics Cost Integration (ai-sdk sink + read-API display)](features/F189-upmetrics-cost-integration.md) | Superseded by F190 | 2 | [plan](features/F189-upmetrics-cost-integration.md) |
| F190 | [Adopt @broberg/ai-sdk as Trail's discrete-LLM layer](features/F190-adopt-ai-sdk-llm-layer.md) | Planned | 2 | [plan](features/F190-adopt-ai-sdk-llm-layer.md) |
...
Per Christian's standing policy (alt LLM gennem `@broberg/ai-sdk`), ruter F190 ALLE **diskrete** LLM-kald i Trail gennem én delt `createAI()`-klient (`ai.vision/chat/translate/…` + `upmetricsSink(agentName:"trail")`), og retirerer den hjemmerullede provider-plumbing, fallback-chains og cost-parsing. **Cloud = HTTP/API-transport** (metered — accepteret; `claude` CLI findes ikke på Fly, så det er også en prod-readiness-gevinst). **Den agentiske ingest-MCP-loop FORBLIVER claude-code-orkestrering** (ai-sdk's ruling #2548: facaden er single-shot, ikke et agent-runtime). 5 stories: F190.1 vision → F190.2 helpers (translate/tag/source/glossary/lint) → F190.3 chat med Trail-ejet tool-loop → F190.4 oprydning + Trail upmetrics-key → F190.5 cost-display i F151-panelet. **Absorberer F189** (sink falder ud gratis; read-API-display bliver F190.5). Den lokale Max-Plan-$0-engine er et separat fremtidigt plan-kort (idé `019e87f9`). Medium-Large (~4-6 dage).

### F189 — Upmetrics Cost Integration (superseded by F190)
To additive, flag-gatede lag oven på F151. **Sink:** Trail's LLM-kald rutes gennem `@broberg/ai-sdk` (seam = `services/claude.ts spawnClaude` + OpenRouter-pathen) så per-kald cost (tokens + `cost_usd` + provider/model/tier + `transport`) lander i upmetrics' `agent_runs`. Fire-and-forget — må aldrig kaste ind i LLM-pathen (samme garanti som F182.4 `recordReinforcement`). `metered:false` ⇔ `transport='subprocess' || cost_usd=0` (Max-Plan-skel). **Display:** ny `services/upmetrics-cost.ts` read-client kalder upmetrics' F014 `GET /api/cost/summary` + `/api/cost/timeseries` (auth = per-projekt `X-Upmetrics-Key` Fly-secret, ikke DSN) og F151 Cost-panelet viser upmetrics-totaler SIDE OM SIDE med internt `ingest_jobs.cost_cents`. Krydscheck: `upmetrics_micro_usd/10_000 ≈ trail_cost_cents` (Trail lidt lavere pga. per-row `Math.round` sub-cent-tab). Non-goals: fjerner IKKE intern cost-capture (source-of-truth + Max-Plan-fallback), ingen credits/billing (F156's domæne). **BLOKERET** på: `@broberg/ai-sdk`-sink skal lande rigtige cost-rows via xrt81-piloten + upmetrics F014.1 skal shippe. Trail er freeze-gate-reviewer for upmetrics F014.1 (grøn-lyser deres `/api/cost/summary`-JSON mod `CostSummary`-typen i `cost-aggregator.ts` før freeze). Intercom-tråd #2379→#2381→#2415→#2416→#2417. Medium effort (3-4 dage, mest kalendertid er de eksterne blockers).
