...
> **Amended 2026-08-23 — "the live site is fine" is true only of content.**
> The Verification section below was finally run, and it failed. Content is in
> sync and live (2 pages, 9 posts, all 200). Landing *code* is not: `/pricing`
> is generated by `build.ts`, was pushed live once, and was reverted by the next
> webhouse.app deploy, which rebuilds from the CMS's own older copy of the code.
> The repo's landing code has had no route to production since the Fly path
> broke. See **F203.2**. This does not change the open question above, but it
> does change the urgency — the dead workflow is not only noise.
...
- **F203.1** — Resolve the landing deploy path and leave CI green (carries the
  acceptance criteria).
- **F203.2** — Landing code cannot reach production, and the CMS deploy reverts
  it. Found by running F203's own Verification step.
