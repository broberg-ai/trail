...
## SUBMITTED — 2026-08-27 20:06 CEST

**Extension ID: `acfimpoddmjlpdlngennokdemdnghjgj`** (permanent; Chrome derives
it at item creation, not from the package). Store URL once approved:
`https://chrome.google.com/webstore/detail/acfimpoddmjlpdlngennokdemdnghjgj`

Submitted by Christian in the dashboard as cb@webhouse.dk; visibility
**Unlisted**. Google warns the host permission "may require an in-depth review
that will delay publishing" — expect longer than the usual few days.

### Four things were wrong at submission time and were fixed live

Recorded because each of them would have stopped or degraded the submission,
and none was visible without measuring:

1. **The zip was stale.** Built 23 Aug 14:49, i.e. BEFORE the custom
   knowledge-base picker (a5c7c7c) and the manifest gate (3a3a65d). Measured on
   the extracted bundle: the old zip carried a native `<select>` and none of the
   `role=listbox` markers. Rebuilt 27 Aug 18:15 — verified both directions
   (native select absent, listbox present, 13 data-testid). The screenshots were
   taken AFTER the picker landed, so shipping the old zip would have meant store
   images showing a control that is not in the package — grounds for rejection.
2. **The summary was 135 characters against a 132 limit**, and the doc claimed
   131. The dashboard rejects at the counter. Reworded to a measured 131 with
   the same content. Every other field was measured at the same time; all were
   comfortably inside their limits.
3. **"Are you using remote code?" defaulted to YES.** Untrue for us — everything
   is in the package, nothing is fetched at runtime — and it demands an extra
   justification while inviting deeper review. Set to No.
4. **The promo tiles were near-white (`#faf9f5`).** Google's own listing guide
   says to avoid "excessive white/light gray" and "undefined edges", and to use
   saturated colours. Rebuilt on the extension's dark theme (`#1a1715`) with the
   peach accent bar, which also matches what the screenshots actually show —
   the popup is dark.

### Store assets, as submitted

All in `apps/web-clipper/store-screenshots/`, all measured for size + alpha:

| File | Field | Size | Alpha |
|---|---|---|---|
| `store-icon-128.png` | Store icon | 128×128 | yes (permitted) |
| `01-popup.png` | Screenshot 1 | 1280×800 | no |
| `02-settings.png` | Screenshot 2 | 1280×800 | no |
| `03-in-context.png` | Screenshot 3 | 1280×800 | no |
| `promo-small-440x280.png` | Small promo tile | 440×280 | no |
| `promo-marquee-1400x560.png` | Marquee promo tile | 1400×560 | no |

Generated by `/tmp/mk-tiles.py` at the time; if the tiles ever need regenerating,
the recipe is: dark `#1a1715` ground, `icons/icon128-dark.png` at left, peach
`#e8a87c` bar down the left edge, Helvetica Neue Bold name + Regular tagline,
both auto-shrunk to fit with a full-pad right margin.

Data-usage disclosures ticked: **Authentication information** (the user's own
Trail token, stored locally) and **Website content** (page text, only on click).
Nothing else. All three certifications ticked truthfully.

### Watch for this

If Distribution was left on "publish manually after review" rather than
publishing straight away, the approved item **expires 30 days after passing
review** — the dialog says so. Verify the item goes live once approved rather
than sitting staged.
