...
- **F190.4** — ✅ (core): `UPMETRICS_API_KEY` set on trail-engine-001 + deployed, sink live, real cost confirmed. **spawnClaude sweep done (2026-06-03):** `action-recommender.ts` was the last discrete-call site still shelling out to the `claude` CLI — broken on the cloud engine ("claude not found", every recommendation silently failed). Migrated to `ai.chat()` (anthropic→openrouter, transport:http) + `labels:{tenantId,kbId}`. **Live-verified:** the exact 6 candidates that logged "0 done, 6 failed" pre-fix now log "6 done" in 13s post-deploy. Only remaining `spawnClaude` is the intentionally-kept ingest claude-cli backend (Max-Plan localhost). **Follow-up still open:** sweep orphaned helper consts; retire the F159 chat backends + their chat-settings config UX (chat-settings.ts still imports `resolveChatChain`/`ChainStep` → a UX change, not just deletion). (Stale comment in `queue-backfill.ts:17` says translation "spawns a claude -p" — translation migrated in F190.2; cosmetic.)
