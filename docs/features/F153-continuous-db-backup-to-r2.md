...
2. **Decide + implement the interface surface** — either add `snapshot(outDir)` to `TrailDatabase` (clean abstraction) OR expose `client` as a typed accessor on `LibsqlTrailDatabase` only (smaller blast radius, avoids touching the interface). Locked in Open Questions below. Update the dry-run script to exercise whichever shape wins.
