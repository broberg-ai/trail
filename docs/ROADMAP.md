...
| F165 | [Async Vision-describe (move out of upload critical path)](./features/F165-async-vision-describe.md) — upload-pipelinen kalder ikke længere Anthropic Vision inline pr. body-image; den ekstraherer text + PNG'er, persister `document_images` med `vision_description=NULL`, returnerer 201 hurtigt, og auto-submitter et `vision-rerun` job. F164's job-framework (pLimit(4) + SSE-progress + zombie-recovery) tager over. Tekst kompileres straks til Neuroner; vision-beskrivelser landes i baggrunden over 30-60s. Default `TRAIL_PDF_TIMEOUT_MS` bumpes til 240 000 som band-aid mens F165 implementeres. **Løser**: 42-side `De_helbredende_urter.pdf` (149 botanical illustrations) ramte 120s timeout efter ~220s vision-tid og lod 149 orphan PNG'er på disk uden DB-rows; nu lykkes upload på <10s og vision trickler ind via progress-modal. | F161, F164 | Small |
| F165.1 | [Vision derivatives (WebP) + strict-availability fallback](./features/F165.1-vision-derivatives-webp.md) — Anthropic's 5 MB image-payload limit får phone-photos og high-res scans til silently at fejle Vision (vision_description=NULL forever). F165.1 introducerer **derivater**: ny `document_images.vision_derivative_path` kolonne (migration 0026) + `ensureDerivative()`-service der genererer en WebP @ q=80 max-dim 1568px ved siden af originalen ved første vision-rerun. Original-PNG/JPG forbliver byte-uændret i storage — Trail er ved at blive customer's **Brand Intelligence Base** så lossless-bevarelse af råmateriale er hård regel. Threshold: file_size > 3 MB ELLER pixels > 4M; under tærsklen skipped derivat-generering helt. `sharp` som ny runtime-dep (~25 MB image bump, OK). Samtidig strammes F164 Phase 3 fallback-semantik: OpenRouter-fallback kun ved 5xx/timeout/connection-fejl — 4xx (inkl. 413 image-too-large) kastes nu eksplicit i stedet for stille at routes til OpenRouter (skjuler bugs som "OpenRouter usage up 100%" der reelt var input-shape-drift). Opt-in backfill-script for eksisterende > 5 MB billeder. **Løser**: Sannes kommende behandlings-foto-upload-flow + alle foto-tunge sources hvor Anthropic stille har svigtet siden F164 Phase 3 inverterede provider-chain'en. | F161, F164 Phase 3, F165 | Small-Medium |
