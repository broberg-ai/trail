...
- **F185** ([`docs.trailmem.com` — AI-friendly technical reference](./features/F185-trail-docs-site.md)) — public, EN-only docs site på samme mønster som `cms-docs` / `docs.webhouse.app`: standalone Next.js Fly app (`trail-docs` i broberg-ai/arn), content i `apps/docs/content/` som `@webhouse/cms`-collections (`docs`/`changelog`/`snippets` med `category`-field for sidebar-grouping), GHA-deploy on push til `main`. **Ikke** webhouse.app/admin-managed — cms-core verificerede 2026-05-08 at docs.webhouse.app heller ikke er det (Christians initial recollection var forkert), så docs-sites følger "developer-content lever ved siden af koden"-mønstret indtil F147 webapp blueprint contract lander. Lukker to vedvarende pain-points for Christian: (1) planning-AI'er som Claude Desktop er upålidelige på GitHub MCP selv på public repos, (2) defaulter til "lad os lave RAG med embeddings" når de skal designe memory-layer fordi trails compile-at-ingest-model er usynlig før de læser sourcen. Phase 1 (3-4t i dag): scaffolding + 4 sider (`/`, `/intro`, `/why-not-rag`, `/quick-start`) + Fly app + Cloudflare DNS + GHA workflow. Phase 2 (1d): concepts (Neurons/queue/KB/connectors/search). Phase 3 (1d): hand-written `packages/shared/openapi.yaml` + engine `/api/v1/openapi.json` + Redoc embed på `/api-reference`. Phase 4 (½d): MCP integration guide (claude-code/claude-desktop/cursor). Phase 5 (½d): `/llms.txt`, `?format=md`-query-param, contract-test (live-route ↔ spec-diff), Pagefind static search. Stack: Next.js 16 SSR, `@webhouse/cms`, Shiki for code blocks, Redoc CDN bundle for OpenAPI viewer (matches cms-docs). Phase 1, Small 3-4t.
