...
| F22 | ~~Stable `{#claim-xx}` Anchors~~ — **Done 2026-05-03** (`prepareCompiledMarkdown` injects `{#claim-XXXXXXXX}` markers post-LLM compile; admin's `markdown.ts` strips markers + emits `<a class="claim-anchor" href="#claim-xxx">#</a>` icon w/ hover-tooltip; `stripClaimAnchors` in @trail/core wired into `chat.ts` (input + output) and `retrieve.ts` (F160) so markers NEVER leak to LLM context or chat answer; per-device toggle in Settings → Visning, default OFF; F22.1 adds sticky toggle in wiki-reader header for in-flow flips; 13/13 leak-prevention assertions green) | F07 | Small |
| F22.1 | ~~Sticky claim-anchor toggle in wiki-reader header~~ — **Dropped 2026-05-03** (built then immediately reverted same day. Claim-anchors are an internal cross-reference primitive — for end-users they're noise, for power-users browser dev-tools or right-click → Inspect already exposes the `id=` attribute. The toggle solved a problem nobody actually had. Settings-level toggle stays for the rare power-user who wants them globally visible. Plan-doc kept for institutional memory: "we tried it, here's why we stopped.") | F22 | Tiny |
...
| F151 | ~~Cost & Quality Dashboard~~ — **Done** (audit 2026-05-03 fandt allerede shipped: `cost-aggregator.ts` 6 exports + `routes/cost.ts` 4 endpoints + `panels/cost.tsx` (482 lines) + `quality-compare.tsx` + migrations 0016 date-index + 0017 cost_estimated. Cost-tab på `/kb/:kbId/cost` + Quality-tab på `/kb/:kbId/sources/:sourceId/compare`. Plan-doc nævnte F151 som "Phase 1 cost / Phase 2 quality" men begge tabs landede sammen) | F149, F143, F148 | Medium |
...
| F153 | ~~Continuous online backup of `trail.db` to Cloudflare R2~~ — **Done 2026-05-03** (Phase 1 `dbcbe22`: VACUUM INTO snapshot primitive; Phase 2 `5acd290`: R2 provider + manual POST + 14-assertion verify; Phase 3 `273c456`: retention pruning + lint-scheduler plug-in @ 24h cadence + 20-assertion verify; Phase 4 read-only status card on `/settings` via `GET /api/v1/backups/health` — auth-split keeps `/admin/backups*` owner-only and the health-aggregate any-auth so tenants get peace-of-mind without trigger-power) | — | Small |
...
| F159 | ~~[Pluggable Chat Backends](./features/F159-pluggable-chat-backends.md)~~ — **Done 2026-05-13** (backend shipped earlier: `ChatBackend` interface + 3 implementations (`claude-cli-backend.ts`, `claude-api-backend.ts`, `openrouter-backend.ts`) in `apps/server/src/services/chat/`, `resolveChatChain()` chain-resolution, `runChat()` runner in `index.ts`, per-KB `chat_backend` + `chat_model` + `chat_fallback_chain` columns on `knowledge_bases`, route `/chat-settings` GET+PATCH. 2026-05-13 added UI piece: `packages/shared/src/chat-models.ts` whitelist of 7 chat models across 3 backends, `getChatSettings`/`updateChatSettings` in `apps/admin/src/api.ts`, parallel `chatSettings`/`selectedChatModelKey`/`chatSavePending` state in `settings-trail.tsx` mirroring F152 ingest-model dropdown verbatim, effective-fallback-chain preview line, independent save-button, locale strings in da/en. Unblocks F33 (Fly.io prod chat no longer needs `claude` CLI on disk — engine can run pure-API backends). | F149, F144 | Medium |
...
- **F176** ~~([per-KB lint schedule + settings UI](./features/F176-per-kb-lint-schedule.md))~~ — **Done** (audit 2026-05-13 found already shipped — migration 0031 added `knowledge_bases.lint_schedule_days` column (1-90 range, NULL = use env-var fallback), Drizzle schema + index, lint-scheduler reads per-KB cadence + computes nextDueAt from `activity_log` lastPassAt (engine-restart-resilient), boot-delay reduced 4h → 5min via `TRAIL_LINT_INITIAL_DELAY_SECONDS=300`, global default `TRAIL_LINT_SCHEDULE_DAYS=7` flytter default fra daily til weekly, legacy `_HOURS` env-var retained for backward compat. PATCH `/api/v1/knowledge-bases/:id` accepts `lintScheduleDays`; admin's `settings-trail.tsx` has dropdown + fetches lint-status from `GET /knowledge-bases/:kbId/lint/status` for last-pass/next-scheduled status card. Phase 1, Small 1d.
...
- **F178** ~~([landing build automation + auto-deploy](./features/F178-landing-build-automation-auto-deploy.md))~~ — **Done 2026-05-03**: `apps/landing/package.json` har `build` (BUILD_OUT_DIR=deploy locked) + `ship` scripts; `build.ts` validerer posts mod streng Zod-schema og fejler loud på manglende/forkerte felter; `.github/workflows/landing-deploy.yml` auto-deployer på push til main når `apps/landing/{content,public,build.ts,Dockerfile,fly.toml,package.json}` ændres. Alle 9 posts passerer schema-validering ved første kørsel. Lukker de fire fejl-modes der ramte 2026-05-02-session. Krav: `FLY_API_TOKEN` secret skal sættes i repo Settings → Secrets → Actions før workflow virker.
...
- **F181** ([Trail Routines](./features/F181-trail-routines.md)) — user-authored markdown filer i `/routines/` per KB der declarer schedule + deterministic query (FTS5/SQL) + LLM synthesis-prompt + write-target. Scheduler tikker hvert 5. min, henter due routines, runner deterministic-step → latent-step → emitter candidate til F143 queue (aldrig direkte write). F19 confidence-policy + F106 Solo Mode governer auto-approval — curator stays in control. Idempotency via F158-style signature-skip. Cost-controlled per F156 + F149 + F179 (default `gemini-2.5-flash` via OpenRouter, Pro tier kan opt'e ind på Sonnet). Lukker action-layer-gap'et identificeret af Yanli Liu's 2026-04 "RAG, LLM Wiki, or GBrain" artikel — Trail's compile-and-query bliver til compile-query-and-let-the-system-do-recurring-synthesis-while-you-sleep. Interim plan-doc med 7 åbne scope-spørgsmål øverst. Phase 1 (foundation) 3d + Phase 2 (admin UI) 2d + Phase 3 (polish) 1-2d. **Reserved planned, deferred to post-Sanne-launch** — feature-creep mod product-market-fit hvis vi shipper det inden basic compile-and-query er rock-solid for én betalende tenant.
- **F182** ([Memory Lifecycle](./features/F182-memory-lifecycle.md)) — per-Neuron `confidence`-kolonne (REAL [0,1]) der dekayer over tid via tunable τ per Neuron-type (concept τ=365d, session τ=30d, etc), forstærkes på reinforcement-signaler (cite, access, chat-ref, curator-pin), og auto-superseder på F158 contradiction når delta > 0.25 + source-count strengthens. Ny `confidence_signals`-tabel logger reinforcement-events. Decay-job (F32-pattern) recomputerer nightly. Reader graph dimmer low-confidence noder, F89 chat-context booster high-confidence først. Admin "Memory Health"-tab viser confidence-histogram + decaying-list + per-type τ-tuning. Generaliserer F139 heuristic-decay til alle Neuron-types og lukker "knowledge rots over time"-gap'et fra Rohit Ganapathy's *LLM Wiki v2* gist. Interim plan-doc med 7 åbne formel/threshold-spørgsmål øverst. Phase 1 (foundation 4d) + Phase 2 (supersession + UI 3d) + Phase 3 (per-claim, deferred). **Reserved planned, deferred to post-Sanne-launch Phase 2** — foundational for F183/F184; Sanne's KB er ideal first calibration testbed.
- **F183** ([Consolidation Tiers](./features/F183-consolidation-tiers.md)) — fire-tier memory model fra cognitive science: working / episodic / semantic / procedural. Ny `tier`-kolonne på `documents` orthogonal til F101 type. Auto-promotion-jobs: working→episodic på session-end (F39 trigger eller 8h idle), episodic→semantic ved embedding-clustering, semantic→procedural ved repeated-pattern-extraction. Promoteringer emitter candidates til F143 queue (curator-mediated per F19 + F106). Tier-aware F89 chat retrieval booster højere tiers først (procedural > semantic > episodic > working). Reader F99 graph viser tier som node-shape + farve. Admin Consolidation-tab viser tier-distribution + pending promotions + manuel trigger. F139 heuristics grandfathers som `tier='procedural'`. Interim plan-doc med 6 åbne triggertresheld-spørgsmål øverst. Phase 1 (3d foundation) + Phase 2 (2-3d promotions + UI) + Phase 3 (open-ended tuning på Sanne's KB). **Reserved planned, depends on F182** — tier-boosting kræver confidence-metadata. Trail bliver første hosted SaaS der productizer cognitive-memory-modellen — marketing-angel for fremtidig landing-post om hvorfor Trail's memory ligner din hjerne.
- **F184** ([Entity Layer + Knowledge Graph Queries](./features/F184-entity-layer-graph-queries.md)) — første-klasses `entities`-tabel + LLM-extraction-step i F06 ingest pipeline + entity-aware F89 chat tools. Ny vocabulary-blok i F140 `_schema.md` declarerer KB-specifikke entity-typer (Sanne: treatment/condition/point/meridian; tech-KB: library/framework/decision/file/person) + attribut-skemaer. Tre tabeller: `entities` (id+kb+type+slug+attributes_json), `entity_mentions` (neuron↔entity bidirectional), `entity_relationships` (typed edges mellem entities, parallel til F137 men entity-level). Nye MCP tools: `entity_lookup`, `entity_neighbors(depth, edge_types)`, `entities_matching(type, attribute_filter)`. Reader entity-sidebar grupperer mentions per type. Admin entity-browser med bulk-merge UI. Stay-in-trail.db (recursive CTEs) for nu — Neo4j-integration er Phase 4 for >50K-Neuron tenants. Lukker "graph traversal queries"-gap'et fra Rohit's v2 + matcher Cornell-paper's 3x accuracy-lift på KG-grounded Q&A. Interim plan-doc med 8 åbne taxonomy/dedup-spørgsmål. Phase 1 (5d foundation) + Phase 2 (5d UI + advanced) + Phase 3 (3-5d bilingual + tuning) + Phase 4 (separate F-number, ekstern graph DB). **Reserved planned, depends on F182 + F183** — entities skal have lifecycle-metadata. Største architectural shift af v2-features (10-14d Phase 1+2 vs F182/F183's 5-10d).
- **F186** ([Design 1:1 Port (Claude Design → Trail Admin)](./features/F186-design-1to1-port.md)) — komplet visuel port af Claude Designs `docs/design/trail_app/` (10 spec-sektioner, 6095 JSX-linjer) til vores Preact/Tailwind/shadcn-stak. 1:1 visuelt (pixels, spacing, tokens, copy, states), men inline-`style={{...}}`-blokke oversættes til Tailwind-utility-classes og shadcn-byggeklodser. Inner-trail sidebar erstatter 11-tab horizontal-stripen (alle 21 panels mappes til 4 grupper Brug/Kanon/Pipeline/Input + footer Omkostning+Indstillinger), Cmd+K command palette tilføjes (ny feature), `/settings` (Account Preferences) + `/tenants` (Manage) bliver dedikerede ruter, TenantSwitcher pille gør multi-tenant synlighed mulig fra dag 1 (`cb@webhouse.dk` skal se både `sanne-andersen` + `broberg-ai`), empty-states på alle 21 panels via shared `<EmptyState>`, mobile bottom-sheets via `<Sheet>`-primitive. Stack uændret (Preact 10 + Vite + Tailwind v4 + shadcn). Tweaks-panel droppes, phone-frame wrapper droppes (vi vil have ægte responsive breakpoints). Plan-billing-UI skjules indtil F40.2b lander. Invitations + personal API keys er stub'et med "Coming Soon" og kommer i F187+F188. 5 faser (A: Chrome/sidebar shell ~2-3d, B: Login+Home+EmptyStates ~1-2d, C: Cmd+K palette ~1-2d, D: /settings + /tenants ~2d, E: Polish ~1-2d). Total ~5-7d fordelt over flere sessioner. Phase 1, Large.
- **F187** ([Tenant Invitations](./features/F187-tenant-invitations.md)) — invitations-tabel + email-flow (Resend) + accept/decline/revoke endpoints + UI-port af F186's Manage Tenants Invitations-fane fra "Coming Soon" til funktionel. Spawnet af F186 (designet kræver det). Interim plan-doc med 7 åbne spørgsmål øverst (invite-mekanik, email-transport, role-tildeling, expiry-window, revoke-policy, duplicate-invite-handling, audit-logging) der skal afklares før spec lockes. Phase 2, Medium 2-3d.
- **F188** ([User-level Personal API Keys](./features/F188-user-api-keys.md)) — `user_api_keys`-tabel i `control.db` med dual-write til `key-index.db` (F40.2a routing-pattern) + endpoints + Account Preferences Developer-sektion-port fra "Coming Soon" til funktionel tabel + one-time secret-reveal-modal ved generering. Cross-tenant scope (user-level, ikke tenant-level som F-eksisterende bearers). Spawnet af F186 (Developer-sektion i designet kræver det). Interim plan-doc med 7 åbne spørgsmål øverst (auth-model, tenant-scope-resolution, permission-model, audit-distinction, rate-limit-tier, revoke-flow, CLI-tooling). Phase 2, Medium 2d.
- **F185** ([`docs.trailmem.com` — AI-friendly technical reference](./features/F185-trail-docs-site.md)) — public, EN-only docs site på `docs.trailmem.com` (Fly app `trail-docs` i broberg-ai/arn, custom-static SSG matching `apps/landing` 1:1, Redoc CDN bundle for OpenAPI viewer). **Phase 1 ✓ Done 2026-05-08** (scaffolding + 4 sider `/`, `/intro`, `/why-not-rag`, `/quick-start` + Fly app + Cloudflare DNS + Let's Encrypt cert + GHA `docs-deploy.yml`). **Phase 1.5 ✓ Done 2026-05-08** (visual brand 1:1 med landing/admin — nav-block lifted verbatim, neuron-particle baggrundsanimation + dark/light theme-toggle + hamburger mobile-menu fra landing's eksakte `<style>`+`<script>`-blokke). **Phase 2 ✓ Done 2026-05-12** (5 concept-sider Neurons/Queue/KB/Connectors/Search med `category`-grouped sidebar — Start here / Concepts / Audiences / Patterns / API). **Phase 3 ✓ Done 2026-05-12** (hand-written `packages/shared/openapi.yaml` for de 4 external-facing endpoints retrieve/queue-candidates/chat/search + interaktiv Redoc-viewer på `/api-reference/` + raw YAML på `/openapi-trail.yaml`). **Extra ✓** `/who-is-trail-for/` (11 audience profiles for sales) + `/site-llm-with-trail-as-tool/` (Pattern C case study fra sanneandersen-deployment). **Phase 4 ⏭ Planned** (½d: MCP integration guide for claude-code/claude-desktop/cursor). **Phase 5 ⏭ Planned** (½d: dynamic `/llms.txt`, `?format=md`-query-param, contract-test live-route ↔ spec-diff, Pagefind static search). Lukker to vedvarende pain-points: planning-AI'er som Claude Desktop er upålidelige på GitHub MCP selv på public repos + defaulter til RAG når de designer memory-layer fordi compile-at-ingest-modellen er usynlig før de læser sourcen. Stack-deviation fra cms-docs's Next.js SSR: vores Phase 1+ behov er rent statisk (Redoc er client-side, llms.txt er build-time, ingen i18n), så apps/docs/build.ts mirrors apps/landing's tsx + marked + shiki + nginx-static-pattern. Phase 1, Total ~10t fordelt.
