/**
 * F199.10 — comparison run on DeepSeek v4 Pro (via OpenRouter), Christian's
 * explicit ask ("kør den helt samme test på DeepSeek 4 Pro" despite it NOT
 * being EU-hosted — a curiosity/baseline comparison, not a candidate for the
 * ingest default given the GDPR requirement). Reuses the EXACT same real
 * Sanne source excerpt + fact ledger as verify-f199-10c so the comparison is
 * apples-to-apples with the Mistral small/large results.
 *
 * NOT a GDPR-safe candidate (DeepSeek/China) — this is purely a reference
 * data point for "does the small-beats-large pattern hold on other model
 * families too, or is it Mistral-specific?"
 *
 * Run from apps/server: set -a; source ../../.env; set +a; bun run scripts/verify-f199-10d-ingest-deepseek-comparison.ts
 */
...
if (!process.env.OPENROUTER_API_KEY) { console.error('✗ no OPENROUTER_API_KEY in env'); process.exit(1); }
...
const T = 't-f199-10d', U = 'u-f199-10d';
const DB_PATH = join(process.env.TMPDIR ?? '/tmp', `f199-10d-${process.env.USER ?? 'x'}.db`);
try { rmSync(DB_PATH, { force: true }); } catch { /* first run */ }
const trail: TrailDatabase = await createLibsqlDatabase({ path: DB_PATH });
await trail.runMigrations();
await trail.initFTS();
await trail.db.insert(tenants).values({ id: T, slug: 'f199-10d', name: 'F199.10d', plan: 'hobby' }).run();
await trail.db.insert(users).values({ id: U, tenantId: T, email: 'f199-10d@local.trail', displayName: 'F199.10d', role: 'owner', onboarded: true }).run();
...
console.log('Model: deepseek/deepseek-v4-pro via OpenRouter (NOT EU-hosted -- reference comparison only, not a GDPR candidate).\n');
...
async function runIngestLoop(model: string, prompt: string, api: CandidateQueueAPI, maxTurns: number) {
  ...
  const override = { provider: 'openrouter', model, transport: 'http' as const };
  const messages: ChatMessages = [{ role: 'user', content: prompt }];
  let totalCostUsd = 0;
  for (let turn = 1; turn <= maxTurns; turn++) {
    const res = await ai.chat({ messages, tools, override, temperature: 0.3, maxTokens: 4096, purpose: 'f199.10d-verify' });
...
async function testModel(model: string) {
  ...
  const api = createCandidateQueueAPI({ trail, tenantId: T, tenantName: 'F199.10d', userId: U, connector: 'api', ingestJobId: null, defaultKbId: kbId });
...
const result = await testModel('deepseek/deepseek-v4-pro');

console.log('=== SUMMARY (real Sanne source, DeepSeek v4 Pro via OpenRouter, NOT EU) ===');
console.log(`${result.model}: turns=${result.turns} cost=$${result.costUsd.toFixed(6)} neurons=${result.neuronCount} recall=${result.recall}/${FACTS.length} missing=[${result.missing.join(',')}]`);
