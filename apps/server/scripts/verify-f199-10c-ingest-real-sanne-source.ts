/**
 * F199.10 — THIRD calibration pass, on a REAL production source (with
 * Christian's explicit go + $5 budget approval). Reads a 15-page excerpt
 * (pages 30-44, ~26k chars) from Sanne Andersen's real "Zoneterapibogen_2026"
 * PDF (124 pages total, fetched READ-ONLY via the cloud API — this script
 * NEVER writes to her live tenant/KB; the excerpt is copied into an isolated
 * throwaway temp DB, same sandboxing as verify-f199-9/10/10b).
 *
 * The full 124-page book is OUT of scope here — a single monolithic tool-loop
 * over 200k+ chars is the exact unsolved problem F142 (chunked ingest) exists
 * for, independent of which model runs it. This tests a realistic ~15-page
 * "monolith-strategy-sized" excerpt (F142's own auto-selector bracket) so the
 * model comparison isn't confounded by the chunking-architecture gap.
 *
 * Fact ledger is built from REAL clinical terms/facts verbatim in the excerpt
 * (acupuncture points, measurements, contraindications) — hard grep-based
 * recall, not a subjective read.
 *
 * Run from apps/server: set -a; source ../../.env; set +a; bun run scripts/verify-f199-10c-ingest-real-sanne-source.ts
 */
...
const T = 't-f199-10c', U = 'u-f199-10c';
const DB_PATH = join(process.env.TMPDIR ?? '/tmp', `f199-10c-${process.env.USER ?? 'x'}.db`);
try { rmSync(DB_PATH, { force: true }); } catch { /* first run */ }
const trail: TrailDatabase = await createLibsqlDatabase({ path: DB_PATH });
await trail.runMigrations();
await trail.initFTS();
await trail.db.insert(tenants).values({ id: T, slug: 'f199-10c', name: 'F199.10c', plan: 'hobby' }).run();
await trail.db.insert(users).values({ id: U, tenantId: T, email: 'f199-10c@local.trail', displayName: 'F199.10c', role: 'owner', onboarded: true }).run();

// Real clinical terms/facts verbatim in the 15-page excerpt (pages 30-44 of
// Zoneterapibogen_2026.pdf), spread across foot-diagnosis + acupuncture-point
// sections so a good compile must cover BOTH topics, not just the start.
...
console.log(`Source: REAL excerpt (Zoneterapibogen_2026.pdf p.30-44), ${SOURCE_TEXT.length} chars, 15 pages, ${FACTS.length} ledgered facts.\n`);
...
async function runIngestLoop(model: string, prompt: string, api: CandidateQueueAPI, maxTurns: number) {
  ...
  for (let turn = 1; turn <= maxTurns; turn++) {
    const res = await ai.chat({ messages, tools, override, temperature: 0.3, maxTokens: 4096, purpose: 'f199.10c-verify' });
...
async function testModel(model: string) {
  ...
  const api = createCandidateQueueAPI({ trail, tenantId: T, tenantName: 'F199.10c', userId: U, connector: 'api', ingestJobId: null, defaultKbId: kbId });
...
console.log('=== SUMMARY (real Sanne source, 15p excerpt) ===');
