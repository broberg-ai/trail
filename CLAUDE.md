...
| Area | Path | Notes |
|---|---|---|
| **Engine** | `apps/server/` | Bun + Hono. The Trail itself: REST (`/api/v1/*`), ingest, chat, search. One `trail.db` per tenant under `/data/<slug>/`. Entry: `apps/server/src/index.ts`. **No MCP** — measured 2026-08-29: no `/mcp` route, no `@modelcontextprotocol` dependency, no import anywhere in `apps/server/src` or `packages/core/src`, and `POST app.trailmem.com/mcp` → 404. This row claimed `POST /mcp` for months; a peer session read it, asked to mount Trail as an MCP, and the answer had to be "that surface does not exist". |
...
| **MCP server** | `apps/mcp/` | Standalone **stdio** MCP, **local disk only** — `index.ts:30` is `createLibsqlDatabase({ path: DEFAULT_DB_PATH })`, no URL and no `fetch` in the file, so it cannot reach a cloud tenant. Pointed at a dev machine it answers confidently with the WRONG tenant's data. **This is not the MCP a remote peer wants** — see the row below. |
| **Trail over MCP, for peers** | buddy's MCP server | **THE working answer to "can I mount Trail as an MCP?"** — `mcp__buddy__trail_search` (read-only) + `mcp__buddy__trail_save`, available in every fleet session that has buddy. Implemented in buddy's `apps/server/src/routes/mcp-server.ts`; it calls `GET /api/v1/knowledge-bases/<kb>/search?q=&limit=` (relevance-ranked, so the F214.2 `cursor` bug does NOT bite it) and needs three env vars **on the buddy daemon**: `TRAIL_INGEST_URL`, `TRAIL_INGEST_TOKEN`, `TRAIL_BUDDY_SESSIONS_KB_ID`. Bound to ONE KB by that last var — read-only, single-KB access by configuration. **Trap:** it sends no `X-Trail-Tenant`, so a `scope=all` token silently reads its HOME tenant. **Second trap, which cost a wrong answer to a peer on 2026-08-29:** unconfigured, the tool still exists and replies `TRAIL_* env vars not configured` — a message that does NOT distinguish "Trail has no MCP" from "not wired in this session". It was read as the first. It means the second. |
