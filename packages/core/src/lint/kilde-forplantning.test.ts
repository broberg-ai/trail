 ...
 *
 * DEN MÅLTE SAG, 15./16. september: fem sider sagde «bygges nu» efter kilden
 * sagde «lanceret» — overview.md, glossary.md, flagskib.md, kilde-Neuronen og
...
import { afhaengigeAf, maerkAfhaengige, rydKildeMaerke } from './kilde-forplantning.js';
...
test('AC#0 DEN MÅLTE SAG: alle fem sider findes — ikke kun den der bærer kildens identitet', async () => {
  ...
  for (const n of ['overview', 'glossary', 'flagskib']) {
    ...
    await citerer(n, 'kilde-bid');
  }

  // 'bid' er den der netop blev kompileret om — den er ajour og undtages.
  const fundne = await afhaengigeAf(trail, T, KB, URL_A, 'bid');
...
test('AC#3 AFHÆNGIGHEDEN SLÅS OP — en side der bare NÆVNER de samme ord røres ikke', async () => {
  ...
  await neuron('anden-kilde', URL_B); // anden kilde
  expect(await afhaengigeAf(trail, T, KB, URL_A, 'bid')).toEqual([]);
});

test('AC#3 — INGEN identitet trækker ikke tilfældige sider med', async () => {
  // «Vi ved ikke hvilken kilde det er» må aldrig matche en anden ukendt. Ellers
  // ville en enkelt genkompilering markere hele den ikke-backfill'ede base.
  await neuron('uden-1', null);
  await neuron('uden-2', null);
  expect(await afhaengigeAf(trail, T, KB, null)).toEqual([]);
  ...
  await kilde('kilde-tom', '');
  await neuron('tom', '');
  await citerer('tom', 'kilde-tom');
  expect(await afhaengigeAf(trail, T, KB, '')).toEqual([]);
});
...
test('flere UDGAVER af samme kilde: en citat-kant til den GAMLE række tæller med', async () => {
  ...
  await kilde('kilde-v1', URL_A);
  await kilde('kilde-v2', URL_A);
  await neuron('overview', null);
  await citerer('overview', 'kilde-v1');
  expect((await afhaengigeAf(trail, T, KB, URL_A)).map((f) => f.documentId)).toEqual(['overview']);
});

test('MÆRKET skrives og LÆSES TILBAGE — antallet er målt, ikke antaget', async () => {
  await kilde('kilde-bid', URL_A);
  await neuron('overview', null);
  await citerer('overview', 'kilde-bid');
  const nu = Date.now();
  const afh = await afhaengigeAf(trail, T, KB, URL_A);
  expect(await maerkAfhaengige(trail, afh, nu)).toBe(1);
...
test('mærket RYDDES når siden selv skrives om — ellers melder den sig bagefter for evigt', async () => {
  ...
  await rydKildeMaerke(trail, 'overview');
...
test('tom liste mærker intet og påstår intet', async () => {
  expect(await maerkAfhaengige(trail, [], Date.now())).toBe(0);
});
